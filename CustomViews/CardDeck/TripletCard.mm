...
typedef NS_ENUM(NSUInteger, TripletCardAttributeName) {
  ...
  TripletCardAttributeNameFilling,
...
- (void)setShading:(TripletCardAttributeValue)filling {
  if (filling < TripletCardAttributeValueMax) {
    _filling = filling;
  }
}
...
- (TripletCardAttributeValue)getAttributeByName:(TripletCardAttributeName)attributeName {
  switch (attributeName) {
    ...
    case TripletCardAttributeNameSymbol:
      return self.symbol;
    case TripletCardAttributeNameFilling:
      return self.filling;
...
+ (BOOL) isTripletMatching:(NSArray *)triplet {
  //For a triplet to be a match it must match w.r.t. all its attributes (number, symbol, filling, color)
  return
  ...
  [TripletCard isTripletMatchingWithRespectToAttribute:triplet attributeName:TripletCardAttributeNameFilling] &&
